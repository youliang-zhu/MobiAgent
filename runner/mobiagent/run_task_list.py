...
用法:
    python run_task_list.py --model mobiagent [--service_ip localhost] [--decider_port 8000] [--grounder_port 8001] [--planner_port 8002]
...
def _save_run_summary(output_root, model, timestamp, overall_start_time, overall_end_time, task_execution_records):
    """生成 Agent 运行时间汇总报告"""
...
def main():
    ...
    parser.add_argument("--model", type=str, required=True, help="模型名称（用于组织输出目录，如 mobiagent, gpt, qwen）")
    ...
    task_list_path = os.path.join(project_root, "runner", "mobiagent", "task_list.json")
    ...
    output_base = os.path.join(project_root, "run_test_data")
    output_root = os.path.join(output_base, args.model, timestamp)
    ...
    # 生成运行时间汇总报告
    _save_run_summary(output_root, args.model, timestamp, overall_start_time, overall_end_time, task_execution_records)
