...
读取待评估数据: workspace/data/raw_runs/<model>/<timestamp>/<app>/<type>/<trace_id>/
评估结果输出到: workspace/data/benchmark_results/<model>/<timestamp>/<app>/<type>/results/<trace_id>/

使用方法：
- 测试单个trace:
  python batch_structural_evaluator.py MobiFlow/task_configs/taobao.json type3:7 \\
      --data-base workspace/data/raw_runs/mobiagent/20251106_162407/taobao \\
      --output-base workspace/data/benchmark_results/mobiagent/20251106_162407/taobao

- 测试某个type下的所有trace:
  python batch_structural_evaluator.py MobiFlow/task_configs/taobao.json type3 \\
      --data-base workspace/data/raw_runs/mobiagent/20251106_162407/taobao \\
      --output-base workspace/data/benchmark_results/mobiagent/20251106_162407/taobao
...
MOBIFLOW_ROOT = PROJECT_ROOT / "MobiFlow"
...
def _test_trace_worker(args):
    ...
    Args:
        args: (config_file, data_base_dir, output_base_dir, task_type, trace_id)
    
    Returns:
        TestResult: 测试结果
    """
    config_file, data_base_dir, output_base_dir, task_type, trace_id = args
    ...
    # 创建 runner 并执行测试
    runner = StructuralTestRunner(config_file, data_base_dir, output_base_dir)
...
class StructuralTestRunner:
    ...
    def __init__(self, config_file: str, data_base_dir: str, output_base_dir: Optional[str] = None):
        """初始化测试执行器
        
        Args:
            config_file: 任务配置文件路径 (如 task_configs/taobao.json)
            data_base_dir: 待评估数据目录 (raw_runs/<model>/<timestamp>/<app>)
            output_base_dir: 评估输出目录 (benchmark_results/<model>/<timestamp>/<app>)
        """
        self.config_file = config_file
        self.data_base_dir = os.path.abspath(data_base_dir)
        self.output_base_dir = os.path.abspath(output_base_dir) if output_base_dir else self.data_base_dir
        ...
        self.base_runner.base_dir = str(MOBIFLOW_ROOT)
        
        print(f"=== 结构化测试执行器 ===")
        print(f"任务名称: {self.config['task_name']}")
        print(f"输入目录(raw_runs): {self.data_base_dir}")
        print(f"输出目录(benchmark_results): {self.output_base_dir}")
    
    def _get_results_dir(self, task_type: str, trace_id: Union[str, int]) -> str:
        """获取指定 trace 的结果目录路径
        
        Returns:
            结果目录的绝对路径，格式: <output_base_dir>/<type>/results/<trace_id>/
        """
        ...
        # 构建路径: output_base_dir/type3/results/7/
        results_dir = os.path.join(self.output_base_dir, task_type, 'results', trace_num)
        return results_dir
    ...
    def test_all_traces_in_type(self, task_type: str, max_workers: int = 2) -> List[TestResult]:
        ...
        if max_workers > 1 and len(traces) > 1:
            ...
            # 准备任务参数
            tasks = [
                (self.config_file, self.data_base_dir, self.output_base_dir, task_type, trace_id)
    ...
    def _print_batch_summary(self, task_type: str, results: List[TestResult]):
        ...
        print(f"\n各 trace 的详细结果已分别保存到:")
        print(f"  {self.output_base_dir}/{task_type}/results/<trace_id>/")
        print(f"{'='*70}\n")
...
class BatchTestRunner:
    ...
    def __init__(self, raw_data_path: str, eval_result_path: str, max_workers: int = 2):
        """初始化批量测试执行器
        
        Args:
            raw_data_path: 待评估数据根目录 (如 workspace/data/raw_runs/mobiagent/20251106_162407)
            eval_result_path: 评估结果输出根目录 (如 workspace/data/benchmark_results/mobiagent/20251106_162407)
            max_workers: 并行 worker 数量，默认 2
        """
        self.max_workers = max_workers
        self.base_dir = str(PROJECT_ROOT)
        
        # 直接使用传入的路径（支持相对路径和绝对路径）
        self.timestamp_dir = os.path.abspath(raw_data_path)
        self.output_timestamp_dir = os.path.abspath(eval_result_path)
        
        if not os.path.exists(self.timestamp_dir):
            raise FileNotFoundError(f"输入数据目录不存在: {self.timestamp_dir}")
        os.makedirs(self.output_timestamp_dir, exist_ok=True)
        
        print(f"\n{'='*80}")
        print(f"批量测试模式")
        print(f"{'='*80}")
        print(f"输入目录(raw_data_path):    {self.timestamp_dir}")
        print(f"输出目录(eval_result_path): {self.output_timestamp_dir}")
        print(f"{'='*80}\n")
    ...
    def _test_single_app(self, app: str) -> List[TestResult]:
        ...
        for type_name in types:
            ...
            output_base = os.path.join(self.output_timestamp_dir, app)
            runner = StructuralTestRunner(config_file, data_base, output_base)
    ...
    def _save_batch_summary(self, all_results: Dict[str, List[TestResult]]):
        """保存批量测试的总汇总报告"""
        summary_file = os.path.join(self.output_timestamp_dir, 'batch_test_summary.txt')
        detailed_file = os.path.join(self.output_timestamp_dir, 'batch_test_detailed.json')
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write(f"批量测试汇总报告\n")
            f.write(f"{'='*80}\n")
            f.write(f"输入目录: {self.timestamp_dir}\n")
            f.write(f"输出目录: {self.output_timestamp_dir}\n")
        ...
        # 生成JSON详细报告
        detailed_data = {
            'raw_data_path': self.timestamp_dir,
            'eval_result_path': self.output_timestamp_dir,
...
示例:
  ...
  python batch_structural_evaluator.py --batch-mode \\
    ...
    parser.add_argument('--data-base',
                       help='待评估数据目录 (如 workspace/data/raw_runs/mobiagent/20251106_162407/taobao)')
    ...
    if args.batch_mode:
        # 批量测试模式
        if not args.raw_data_path or not args.eval_result_path:
            parser.error("批量模式需要 --raw-data-path 和 --eval-result-path 参数")
        
        try:
            batch_runner = BatchTestRunner(args.raw_data_path, args.eval_result_path, max_workers=args.workers)
        ...
        # 创建 runner
        runner = StructuralTestRunner(args.config_file, args.data_base, args.output_base)
