...
使用方法：
- python trace_evaluator.py MobiFlow/task_configs/taobao.json              # 测试所有类型
- python trace_evaluator.py MobiFlow/task_configs/taobao.json type3        # 测试指定类型
- python trace_evaluator.py MobiFlow/task_configs/taobao.json type3:150    # 测试指定类型的指定trace
- python trace_evaluator.py MobiFlow/task_configs/taobao.json 150,151,152  # 测试指定的trace编号
"""
...
class UniversalTestRunner:
    ...
    def __init__(self, config_file: str):
        ...
        # 默认以 MobiFlow 为基础目录，兼容 task_configs 中的相对 rules/data 路径
        self.base_dir = str(MOBIFLOW_ROOT)
...
def show_usage(config_dir: str = str(MOBIFLOW_ROOT / "task_configs")):
    """显示使用说明"""
    print(f"""
通用任务测试执行器使用说明:

1. 测试所有类型:
   python trace_evaluator.py {config_dir}/taobao.json

2. 测试指定类型:
   python trace_evaluator.py {config_dir}/taobao.json type3
   python trace_evaluator.py {config_dir}/xiaohongshu.json type2

3. 测试指定类型的指定trace:
   python trace_evaluator.py {config_dir}/taobao.json type3:150

4. 测试指定trace编号:
   python trace_evaluator.py {config_dir}/taobao.json 150,151,152
