...
def _build_gemini_client(cfg):
    if GeminiAdapter is None:
        raise ImportError(
            "Gemini support requires google-genai. Install dependencies before using --decider_api_type gemini."
        )
    return GeminiAdapter(api_key=cfg["api_key"])
...
if __name__ == "__main__":
    ...
    task_json_path = os.path.join(
        PROJECT_ROOT, "workspace", "benchmark", "configs", "task.json"
    )
