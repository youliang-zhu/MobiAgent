...
### `workspace/data_tools/annotated`

1. `workspace/data_tools/annotated/labeled_data_checker.py`：检查 `collect/manual/data` 下人工标注轨迹的完整性与一致性，重点核对 `actions.json` 和 `react.json` 的步骤对齐、字段完整性与动作参数格式；可选清理 `parse.error`，也可按任务类型批量删除 `react.json` 以便重标。
```bash
# 数据完整性检查
...
2. `workspace/data_tools/annotated/ss_data_generation.py`：把多步任务轨迹拆成单步样本，调用 Gemini 生成单步目标改写与 reasoning 变体，输出到 `workspace/data/training_data/ss_data`，用于增强 SFT 的单步决策训练数据。
```bash
...
2. `workspace/data_tools/sft/sft_data_check.py`：对 SFT 数据集进行训练前质量门禁，检查样本字段、动作分布、重复样本、图片可读性与元信息一致性，并输出样本检查结果。
```bash
...
### `workspace/data_tools`

1. `workspace/data_tools/test_api.py`：快速验证外部模型 API 连通性（当前脚本内含硬编码 key，主要用于本地临时联调）。
```bash
python workspace/data_tools/test_api.py
```

## 2. 数据文件目录

### `workspace/data/training_data`

当前目录用于存放训练数据产物（`ss_data` / `unexpected_data` / `sft_data` / `grpo_data`），本节不包含可执行脚本。

### `workspace/data/raw_runs`

当前目录用于存放待离线评估的原始运行轨迹（Agent 执行产物），本节不包含可执行脚本。

## 3. 训练脚本

### `workspace/training/sft/scripts`

1. `workspace/training/sft/scripts/finetune_lora_vision.sh`：SFT 主训练入口（LoRA 微调），读取 `workspace/data/training_data/sft_data` 的训练/验证集并启动 deepspeed 训练。
```bash
bash workspace/training/sft/scripts/finetune_lora_vision.sh
```

### `workspace/training/sft/deploy`
...
# 保存并部署
python workspace/training/sft/deploy/deploy_lora.py --save --deploy
```
...
### `workspace/benchmark/evaluators`

1. `workspace/benchmark/evaluators/trace_evaluator.py`：离线评估核心脚本，读取单个 app 的 trace 数据，基于 `MobiFlow/task_rules` + OCR/LLM 验证规则计算 `success/score/matched_nodes/reason`。
```bash
python workspace/benchmark/evaluators/trace_evaluator.py \
  MobiFlow/task_configs/taobao.json \
  type3
```

2. `workspace/benchmark/evaluators/batch_structural_evaluator.py`：结构化批量评估脚本，批量读取 `workspace/data/raw_runs/<model>/<timestamp>`，并将评估结果输出到 `workspace/data/benchmark_results/<model>/<timestamp>`。
```bash
...
python workspace/benchmark/runners/run_task_list.py \
  --model qwen2_5_unfinetune_grounder \
...
### `workspace/data/benchmark_results`

当前目录用于存放离线评估结果数据（按模型名与批次组织），例如 `batch_test_summary.txt`、`batch_test_detailed.json` 以及各 trace 的 `results/<trace_id>/summary.txt|detailed.json|run.log`。
