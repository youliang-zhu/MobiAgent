...
'''
python workspace/data_tools/grpo/construct_grpo_augmented.py \
...
def is_first_search_click(step: Dict) -> bool:
    """是否为轨迹第一步点击搜索框。"""
    if step.get("step_idx") != 1:
        return False
    react = step.get("react", {})
    if react.get("function", {}).get("name") != "click":
        return False
    params = react.get("function", {}).get("parameters", {})
    target = params.get("target_element", "")
    reasoning = react.get("reasoning", "")
    text = f"{target} {reasoning}"
    return "搜索" in text
...
def sample_steps(all_steps: List[Dict], total_samples: int, 
                 input_ratio: float = 0.2, pre_input_ratio: float = 0.1, swipe_ratio: float = 0.05,
                 unexpected_scale: float = 0.25, first_step_max_ratio: float = 0.30) -> List[Dict]:
    """按比例采样: input 20%, pre_input 10%, swipe 5%, 其余随机（增强版将 other/unexpected 降为原来的 1/4，并增强第一步搜索点击）"""
    # 分类步骤
    ...
    first_search_steps = [s for s in all_steps if is_first_search_click(s)]
    ...
    print(f"First-step search click candidates: {len(first_search_steps)}")
    ...
    sampled_other_full = random.sample(other_steps, min(len(other_steps), remaining))

    # unexpected(other) 降到原来的 1/4
    reduced_other_count = int(len(sampled_other_full) * unexpected_scale)
    sampled_other = random.sample(sampled_other_full, reduced_other_count) if reduced_other_count > 0 else []

    # 将减少掉的配额补给“第一步点击搜索框”样本，保证总样本数不下降
    freed_slots = len(sampled_other_full) - len(sampled_other)
    if freed_slots > 0:
        if first_search_steps:
            sampled_first_search = random.choices(first_search_steps, k=freed_slots)
        else:
            sampled_first_search = random.choices(all_steps, k=freed_slots)
    else:
        sampled_first_search = []
    
    sampled_steps = sampled_input + sampled_pre_input + sampled_swipe + sampled_other + sampled_first_search

    # 控制第一步样本占比不超过阈值（默认 30%）
    max_first_step = int(total_samples * first_step_max_ratio)
    current_first_steps = [s for s in sampled_steps if is_first_search_click(s)]
    if len(current_first_steps) > max_first_step:
        need_drop = len(current_first_steps) - max_first_step
        kept = []
        dropped = 0
        for s in sampled_steps:
            if dropped < need_drop and is_first_search_click(s):
                dropped += 1
                continue
            kept.append(s)
        sampled_steps = kept

    # 轻量补齐：若样本数不足，优先补非第一步样本
    if len(sampled_steps) < total_samples:
        non_first_pool = [s for s in all_steps if not is_first_search_click(s)]
        fill_pool = non_first_pool if non_first_pool else all_steps
        sampled_steps.extend(random.choices(fill_pool, k=total_samples - len(sampled_steps)))

    # 若超出目标，随机裁剪
    if len(sampled_steps) > total_samples:
        sampled_steps = random.sample(sampled_steps, total_samples)
    ...
    first_search_in_sample = sum(1 for s in sampled_steps if is_first_search_click(s))
    print(f"First-step search stats: augmented={len(sampled_first_search)}, final_in_sample={first_search_in_sample}, ratio={first_search_in_sample/len(sampled_steps):.4f}")
...
def construct_grpo_dataset(data_path: str, out_path: str, factor: float = 0.5,
                           ...
                           unexpected_scale: float = 0.25, first_step_max_ratio: float = 0.30):
    """
    构建 GRPO 数据集
    """
    ...
    sampled_steps = sample_steps(
        ...
        first_step_max_ratio=first_step_max_ratio,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GRPO dataset construction (augmented)")
    ...
    parser.add_argument("--unexpected_scale", type=float, default=0.25,
                        help="Scale for unexpected(other) samples relative to original (default: 0.25)")
    parser.add_argument("--first_step_max_ratio", type=float, default=0.30,
                        help="Max ratio for first-step search-click samples (default: 0.30)")
    ...
    construct_grpo_dataset(
        ...
        seed=args.seed,
        unexpected_scale=args.unexpected_scale,
        first_step_max_ratio=args.first_step_max_ratio,
    )
