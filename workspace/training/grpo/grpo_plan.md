...
```
Decider 起点模型 ：C/decider_lora_2_merged
...
- 恢复 `finetune_grpo.sh` 中 `--max_steps 500`（脚本默认值）
- 其他参数不变
- 输出目录：`/scratch/youliang/models/decider_grpo_1`
...
```bash
conda run -n GRPO python workspace/training/grpo/test_base_model_generation.py \
    --model_path /scratch/youliang/models/decider_grpo_1/checkpoint-100 \
